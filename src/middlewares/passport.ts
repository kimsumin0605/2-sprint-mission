...
export const verifyAccessToken = passport.authenticate("access-token", {
  session: false,
});
  ...
  new LocalStrategy(
    ...
    async (email, password, done) => {
      try {
        ...
        if (!user)
          return done(null, false, { message: "등록된 이메일이 없습니다" });
  ...
  new JwtStrategy(
    ...
    async (req: Request, payload: JwtPayload, done: VerifiedCallback) => {
      try {
        ...
        if (
          !user ||
          user.refreshToken !== req.cookies[REFRESH_TOKEN_COOKIE_NAME]
        ) {
